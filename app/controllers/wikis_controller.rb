class WikisController < ApplicationController
  ...
  def index
    ...
    @public_wikis = policy_scope(Wiki.public_wikis).paginate(page: params[:page], per_page: 5)
    #@private_wikis = policy_scope(Wiki.where(private: true)).paginate(page: params[:page], per_page: 10)
    @private_wikis = policy_scope(Wiki.private_wikis).paginate(page: params[:page], per_page: 5)
  #  authorize @wikis
  end
